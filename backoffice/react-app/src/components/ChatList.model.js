    ...
    constructor() {
        ...
        this.onlineCostumers = [{
            ...
            },
            {
                ...
                dateTime: "12:23",
                text: "Morbi convallis"
            }]
        }]

        this.offlineCostumers = [{
            id: "2",
            name: "Mary",
        ...
        manuh.subscribe(topics.costumers.online._path, "ChatListModel", msg => {            
            // add received costumer to online list
            this.onlineCostumers.push(msg.costumer)
            this.onlineCostumers = this.onlineCostumers

            // remove the costumer - if exists - from offline list
            this.offlineCostumers = this.offlineCostumers.filter(obj => obj.id !== msg.costumer.id)
        })
        
        manuh.subscribe(topics.costumers.offline._path, "ChatListModel", msg => {        
            // add received costumer to offline list
            this.offlineCostumers.push(msg.costumer)
            this.offlineCostumers = this.offlineCostumers

            // remove the costumer - if exists - from online list
            this.onlineCostumers = this.onlineCostumers.filter(obj => obj.id !== msg.costumer.id)
        })        
    }
